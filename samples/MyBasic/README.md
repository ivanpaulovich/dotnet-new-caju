# MyBasic: The Clean Architecture Sample Implementation with .NET Core :cyclone:
[![All Contributors](https://img.shields.io/badge/all_contributors-12-orange.svg?style=flat-square)](#contributors)
[![Build status](https://ci.appveyor.com/api/projects/status/0i6s33kw3y87tkb2?svg=true)](https://ci.appveyor.com/project/ivanpaulovich/clean-architecture-mybasic)
...
**MyBasic** is a virtual Wallet application in which a customer can register an account then manage the balance with `Deposits`, `Withdraws` and `Transfers`.

The MyBasic's demo is hosted on `Heroku` servers and the `Swagger UI` client is available at [https://clean-architecture-mybasic.herokuapp.com/swagger/](https://clean-architecture-mybasic.herokuapp.com/swagger/). It is just beautiful!
[![Swagger Demo](https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-mybasic/master/docs/clean-architecture-mybasic-swagger.png)](https://clean-architecture-mybasic.herokuapp.com/swagger/index.html)
...
## Index of Clean Architecture MyBasic
...
Application architecture is about usage, a good architecture screams the business use cases to the developer and framework concerns are implementation details. On **MyBasic** sample the user can `Register` an account then manage the balance by `Deposits`, `Withdrawals` and `Transfers`.

<p align="center">
  <img src="https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-mybasic/master/docs/clean-architecture-mybasic-use-cases.png" alt=Clean Architecture Use Cases" style="max-width:100%;">
</p>
...
![Register Flow of Control](https://github.com/ivanpaulovich/clean-architecture-mybasic/blob/master/docs/register-flow-of-control.svg)
...
MyBasic uses ideas from popular architectural styles. They Ports and Adapters are the simplest one followed by the others, they complement each other and aim a software made by use cases decoupled from technology implementation details.
...
![Ports and Adapters](https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-mybasic/master/docs/clean-architecture-mybasic-ports-and-adapters.png)
...
<p align="center">
  <img src="https://raw.githubusercontent.com/ivanpaulovich/clean-architecture-mybasic/master/docs/clean-architecture-mybasic-layers.png" alt="Layers" style="max-width:100%;">
</p>
    ...
    public static class VersionedSwaggerExtensions
    {
            ...
            ApiVersionDescription apiVersion)
        {
            ...
            options.SwaggerDoc(apiVersion.GroupName,
                new Info()
                {
                    Title = "Clean Architecture MyBasic",
                        Contact = new Contact()
                        {
                            Name = "@ivanpaulovich",
...
Data Annotations are powerful tool from .NET, it can be interpreted by ASP.NET Core and other frameworks to generate Validation, User Interface and other things. On MyBasic project, Data Annotations are used to create a complete Swagger UI and HTTP Request validation. Of course following the Clean Architecture Principles we need to keep frameworks under control.
...
```c#
public sealed class MyBasicContext : DbContext
{
    public MyBasicContext(DbContextOptions options) : base(options)
    {
...
Run the EF Tool to add a migration to the `MyBasic.Infrastructure` project.

```sh
dotnet ef migrations add "InitialCreate" -o "EntityFrameworkDataAccess/Migrations" --project source/MyBasic.Infrastructure --startup-project source/MyBasic.WebApi
```
...
```sh
dotnet ef database update --project source/MyBasic.Infrastructure --startup-project source/MyBasic.WebApi
```
...
```sh
dotnet run --project "source/MyBasic.WebApi/MyBasic.WebApi.csproj" --Environment="Development"
```
...
```sh
dotnet run --project "source/MyBasic.WebApi/MyBasic.WebApi.csproj" --Environment="Production"
```
...
MyBasic is a cross-platform application, you can run it from Mac, Windows or Unix. To develop new features, you may use Visual Studio or Visual Studio Code :heart:.
...
```sh
dotnet run --project "source/MyBasic.WebApi/MyBasic.WebApi.csproj"
```
...
build_script:
  ...
  - dotnet ef database update --project source/MyBasic.Infrastructure --startup-project source/MyBasic.WebApi
  - dotnet build
  - pushd source/MyBasic.WebApi/
  - dotnet pack --configuration Release
  - popd
test_script:
  - dotnet test tests/MyBasic.UnitTests/MyBasic.UnitTests.csproj
  - dotnet test tests/MyBasic.IntegrationTests/MyBasic.IntegrationTests.csproj
  - dotnet test tests/MyBasic.AcceptanceTests/MyBasic.AcceptanceTests.csproj
deploy_script:
  - docker build -t ivanpaulovich/clean-architecture-mybasic:github .
  - docker login -u="$DOCKER_USER" -p="$DOCKER_PASS"
  - docker push ivanpaulovich/clean-architecture-mybasic:github
  - docker login --username=$HEROKU_USERNAME --password=$HEROKU_API_KEY registry.heroku.com
  - docker tag ivanpaulovich/clean-architecture-mybasic:github registry.heroku.com/$HEROKU_APP_NAME/web
...
WORKDIR /app
COPY --from=build-env /app/source/MyBasic.WebApi/out .
CMD export ASPNETCORE_URLS=http://*:$PORT && dotnet MyBasic.WebApi.dll
```
...
| [Clean Architecture MyBasic](https://www.youtube.com/watch?v=ivAkdJmSqLQ)              | Aug 6, 2019  |
